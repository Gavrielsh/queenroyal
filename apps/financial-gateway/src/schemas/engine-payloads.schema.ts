 ...
 * `Prisma.JsonValue` — effectively `any`. The reconciler and the PSP webhook handler MUST NOT
 * trust that blob: a corrupted/partial row would otherwise crash a replay (or, worse, forward
 * a malformed amount to the ledger). Every payload read out of the journal is therefore
 * re-parsed through one of these Zod schemas before any replay logic runs.
 *
 * Money fields reuse the `lib/money` string contract (`^\d+(\.\d{1,4})?$`) — never a
 * `z.number()`. The inferred output types are structurally assignable to the engine DTOs in
 * `types/true-engine`, so parsed payloads forward verbatim with no `any` casts.
 */
...
/**
 * The self-contained DEPOSIT instruction journaled at purchase time. It carries the PSP intent
 * ref (to poll/correlate), the expected amount (to defend the credit against a tampered
 * webhook), and the exact ledger body to replay on a verified `succeeded`.
 */
 ...
 */
export function parseEngineRequestPayload(type: ReplayableEngineRequestType, payload: unknown): ParsedEnginePayload {
  switch (type) {
    case "BET": {
