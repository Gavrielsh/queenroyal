 ...
 *     `succeeded` event — it stamps the captured `payment_ref` into the ledger metadata for
 *     audit. Safe to call any number of times: the engine de-duplicates on
 ...
 * `purchase.operator_transaction_id` (the engine returns CACHED/GHOST_RECOVERED on a replay),
 * so the webhook handler and the reconciler can both drive it without ever double-crediting.
 */
