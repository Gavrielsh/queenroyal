 ...
 *
 * The problem it solves: when Redis goes down, every Redis-backed call would otherwise block
 * on the driver's per-request retries before failing. For a fail-closed gateway that means a
 * dead Redis stalls — and then 503s — EVERY critical route. The breaker detects the outage
 * after a few failures and then FAILS FAST (no waiting), so callers make an instant policy
 * decision: financial/secure paths reject (503).
 *
 * States:
...
export class CircuitBreaker {
  ...
  /** View of the breaker (advances out of `open` if cooldown elapsed). */
  ...
  /**
   * Run `fn` under the breaker. Throws {@link CircuitOpenError} immediately when open (fail
   * fast); otherwise records the outcome and re-throws any error from `fn`.
   */
...
/**
 * Process-wide breaker shared by ALL Redis interactions (replay nonces, and the rate limiter
 * in a later phase). One shared instance means a single outage is observed consistently:
 * critical callers reject (503), and /api/health can report it.
 */
