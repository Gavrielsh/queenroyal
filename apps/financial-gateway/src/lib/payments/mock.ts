 ...
 * intent captures nothing and returns a `client_secret`; settlement arrives later as a signed
 * webhook (or is observed by polling `retrievePaymentIntent`).
 *
 * `createPaymentIntent` is idempotent on `idempotencyKey` (a real Stripe Idempotency-Key
 * returns the same PaymentIntent), and `parseWebhook` verifies an HMAC-SHA256 signature over
 * the raw body exactly as a real PSP webhook would, so the integration seam is realistic.
 *
 * NOTE: the `Map`s below are this mock PROVIDER's own intent ledger (dev/test only) — they
 * are not the gateway's rate-limit / idempotency / session state, which is Redis-backed and
 * fails closed. A real PSP holds this state remotely.
 */
...
export class MockPaymentProvider implements PaymentProvider {
  ...
  // ── Dev/test helpers: advance an intent's lifecycle deterministically ──────────
