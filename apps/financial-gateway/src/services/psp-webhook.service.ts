 ...
 * itself de-duplicates on `operator_transaction_id`. A re-delivered `succeeded` event therefore
...
export async function handlePspWebhookEvent(event: PspWebhookEvent, traceId?: string): Promise<PspWebhookOutcome> {
  ...
  // STRICT-validate the journaled instruction before replaying it. A corrupted JSONB blob
  ...
  // correlation, so we refuse rather than credit.
  ...
  if (!credit.ok) {
    // Capture is confirmed but the ledger credit failed → leave a retryable FAILED intent. The
    // reconciler re-drives the credit only (never a re-charge); idempotent at the engine.
