 ...
 *
 * These values are *instructions* forwarded to the True Engine. This service never
 * stores or computes the resulting balances — the engine is the sole authority.
 */
...
const BY_ID: ReadonlyMap<string, StorePackageConfig> = new Map(
  STORE_PACKAGES.map((p) => [p.id, p]),
);
